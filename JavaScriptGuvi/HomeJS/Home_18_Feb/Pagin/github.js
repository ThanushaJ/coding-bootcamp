...
console.log(start,end);
  for(var j=start;j<end;j++){
    jsonList += "Id:"+rawData[j].id+"<br>Name:"+rawData[j].name+"<br>email:"+rawData[j].email+"<br><hr>";
  }
 document.getElementById("data").innerHTML = jsonList;
...
prevBut.appendChild(ptext);
document.body.appendChild(prevBut);
prevBut.setAttribute("onclick","pre()");
...
while (i < rawData.length) {
    ...
    btn.setAttribute("onclick", `displayData(${count})`);
    i = i + 5
    count++
